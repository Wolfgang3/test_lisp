...
(defun get-all-asana-projects (&key selected-project-id)
  (setf *obj-list* nil)
  ...
  (let* ((url (concatenate 'string "https://app.asana.com/api/1.0/projects/" asana_proj_id "/tasks"))
   (json (jsown:parse
    (drakma:http-request url
           ...
           (cons "opt_fields"  "completed_at,name,notes,assignee.name" ))))))
  ...
  (let* ((asana-project-main-id (asana-project-main-id (get-dao 'asana-project selected-asana-project-id)))
    ...
    (loop for rec in (jsown:val (get-tasks-completed-since token asana-project-main-id start-date) "data")
       ...
       (let* ((task-date (jsown:val rec "completed_at"))
              ...
              (description (jsown:val rec "notes"))
              ;(completed-by (jsown:val (jsown:val rec "assignee") "name"))
              (web-link (concatenate 'string "https://app.asana.com/0/" asana-project-main-id "/" (write-to-string task-id)))
              )
         (if (not (eq task-date NIL))
             (if (eq (is-task-date-less-then-end-date? task-date end-date) T)
          (setf *obj-list* (append *obj-list* 
                                   (list (make-instance 'task :asana-project-id selected-asana-project-id :release-id release-id :asana-task-id task-id :name task-name :assignee-id (jsown:val assignee-obj "id") :completed-at task-date :completed-by (jsown:val assignee-obj "name") :description description :web-link web-link))))))
        ))
